...
test('the five-area run includes Cooling Works and Cable Yard with a harder rooftop roster', () => {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    for (const stage of [8, 9]) {
      ...
      if (level.setpiece) assert(['cable-yard', 'balance-house'].includes(level.id));
...
test('ordinary saves retain progress, old completed escapes migrate, and new escapes retain all fifteen upgrades', () => {
