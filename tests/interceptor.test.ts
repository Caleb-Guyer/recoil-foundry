...
test('rooftop boss selection preserves other rooms, seeded retries and Daily continuation', () => {
  ...
  assert.deepEqual(g.level, getLevel(day.seed, 11));
...
test('the direct test preserves saves and unlocks, while a real victory leads into the escape and preserves old rooftop practice', () => {
  ...
  assert.deepEqual(g.level, getLevel(entry.seed, 11, undefined, 'boss'));
  const seed = entry.seed;
  g.start(seed, { version: 3, seed, stage: 11, hp: 100, mods: [], kills: 0, elapsed: 0 });
  g.enemies[0].spawn = 0;
  g.hitEnemy(g.enemies[0], 999999);
  step(g, 20);
...
for (const mirror of [false, true]) {
  test(`Interceptor is beatable with an eleven-upgrade gun and normal health, mirror=${mirror}`, () => {
    const g = new Game();
