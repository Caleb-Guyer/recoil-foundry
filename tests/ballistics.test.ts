...
test('Countershot recovery freezes during pause and hitstop and resets on a new room or retry', () => {
  const g = fixture(['countershot']);
  assert.equal(g.ballistics.counterCharge, 1);
  assert.equal(g.ballistics.counterGlint, 0);
  target(g, 1500);
  const b = shot(g, 765, 400, -80, 0, false);
  assert(g.ballistics.reflectRound(b, { ...b.pos }));
  assert.equal(g.ballistics.counterCharge, 0);
  g.shots = [];
  for (let i = 0; i < 24; i++) g.tick(1 / 60, idle);
  assert(!g.ballistics.counterReady);
  near(g.ballistics.counterCharge, 0.5);
  g.setMode('paused');
  g.tick(2, idle);
  assert(!g.ballistics.counterReady);
  near(g.ballistics.counterCharge, 0.5);
  ...
  assert(g.ballistics.counterReady);
  assert.equal(g.ballistics.counterCharge, 1);
  near(g.ballistics.counterGlint, 1);
  g.setMode('paused');
  g.tick(2, idle);
  near(g.ballistics.counterGlint, 1);
  g.setMode('playing');
  for (let i = 0; i < 6; i++) g.tick(1 / 60, idle);
  assert(g.ballistics.counterGlint > 0 && g.ballistics.counterGlint < 1);
  for (let i = 0; i < 6; i++) g.tick(1 / 60, idle);
  assert.equal(g.ballistics.counterGlint, 0);
