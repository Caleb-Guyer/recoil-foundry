...
The owner confirmed that neither a physical controller nor a lower-end laptop is currently available. Those checks remain open. Current automation exposes only the Codex in-app browser; installed Chrome is not independently controllable, and Firefox has no connected browser surface. No Chrome/Firefox, controller, touch or low-end-device certification is claimed. The owner's separate Edge reports and manual confirmations remain the evidence for Edge.
