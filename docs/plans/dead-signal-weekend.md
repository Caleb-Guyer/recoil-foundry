...
Themed update planned against 2.98.1, with target version **3.0.0** and local candidate **3.0.0-rc.1**. The [first playable room](dead-signal-prototype.md), [five-upgrade Subversion family](dead-signal-subversion.md) and [alternate route](dead-signal-route.md) are implemented on `feature/dead-signal` as of 25 September. The [Caller](dead-signal-caller.md), [Switchboard boss](dead-signal-switchboard.md), [six combat arrangements](dead-signal-layouts.md) and [regional audio](dead-signal-audio.md) are implemented. The [balance audit and candidate preparation](dead-signal-balance.md) are complete; public-host acceptance remains open. Target: ready Sunday, 27 September 2026, with public release before 7:00 a.m. Central on Monday, 28 September. The timetable below is a work plan, not a scheduled background job.

**Published:** Dead Signal **3.0.0** is live on GitHub Pages and itch.io, authorized on 25 September ahead of the original release window below. [Publication evidence](../validation/dead-signal-3.0.md) records the build, artifact checksum, saved-progress checks and store announcement. The earlier milestones and schedule below document the implementation history.

**Foundation completed:** one Transmission Annex room in both mirrors; one Switchman and interruptible gun port; pre-equipped Spoof with temporary blue allies; bullet, beam and shell presets. At that milestone, the production build and 1,601 automated tests passed, including 27 prototype checks and six ordinary-input combat completions. Browser presentation, pause and retry were checked in the in-app browser. See the prototype notes for that development preview's scope.
...
**Route integration implemented:** physical Cooling/Annex exits after Furnace; Broadcast Floor, Cable Well and Receiver Gallery in both mirrors; first-room Spoof offer; Continue and pending-reward persistence; regional recap and Logbook discovery. Daily 80 introduced the seeded region while 78/79 retain their old rooms. Annex revision 3 and Daily 82 now end the area with Switchboard in room 12 before returning to Reclamation. Older revisions retain the original shared boss. See [route notes and test links](dead-signal-route.md).

**Caller implemented:** three sampled target positions, a visible lock, and three delayed physical rounds; cover and movement counter it. Cable Well introduces one, and Receiver Gallery pairs it with a Switchman. Spoof reboots a fresh enemy-targeting Caller. The speaker has original maintenance lore, and both mirrors are playable. Daily 81 and saved Annex revisions preserve existing encounters. See [Caller notes and test links](dead-signal-caller.md).

**Switchboard implemented:** three remote emitters, a mobile control assembly, taught attack patterns and a combined second phase; interruptible junctions with a shared cooldown and damage opening; original boss lore and victory-only Practice. Fifty focused tests cover combat, anti-camping probes, collision and progression compatibility. See [boss notes and test links](dead-signal-switchboard.md) for exact validation scope.

**Layout variety implemented:** Crossed Lines, Broken Ladder and Relay Stacks bring the area to six authored combat arrangements, each mirrored, plus its boss arena. Revision 4 and Daily 83 select each room independently from the run seed; older revisions retain their layouts. Ordinary-jump traversal, 36 room-combat completions and six continuous alternate-route completions are verified. See [layout notes and test links](dead-signal-layouts.md).

**Regional audio implemented:** “Orders After Hours,” an original eight-bar synth-rock arrangement with a sparse entrance, responsive combat rhythm, Switchboard lead and phase-two answers, and quiet room-clear hum. Distinct charge, lock, interruption and reboot cues use the existing mix and settings. The actual browser audio graph is measured with and without gunfire; pause is silent and no samples clip. See [audio notes and listening preview](dead-signal-audio.md). The [balance audit](dead-signal-balance.md) is complete; public-host acceptance remains below.
...
**Sound and discovery.** The [regional arrangement and boss layer](dead-signal-audio.md) are implemented within the existing procedural score. Hostile warnings lower music and ordinary effects; friendly cues do not. No licensed-song dependency or new composer delivery is on the weekend's critical path. Listening during the final whole-update playthrough remains part of release acceptance.
...
**Completed validation.** The [balance report](dead-signal-balance.md) replaces the resolved implementation gates: 1,885 passing tests, a production build, 24 paired regional continuations, 50 fresh campaign attempts with 13 full extractions (including two Daily runs), an actual Overtime boarding and first-room clear, 144 stationary probes, 40 build probes, save/Daily compatibility, browser warning inspection, a bounded 2.98.1 performance comparison and repeated-restart caps. Nine pilot navigation timeouts and human/hardware limitations remain explicitly recorded. The final 3.0 suite and build passed again, then the verified release ZIP was published to both hosts. `v2.98.1` remains the immutable rollback reference.

**Release checklist complete.** Completed publication and public-host checks have been removed. The owner authorized launch with the documented limits: independent player feedback, listening approval and physical low-end evidence remain unavailable, not falsely marked passed. Future player feedback should inform patches, especially Annex health cost and modest Burst-beam/shell builds.
...
**Launch presentation — published.** [The launch kit](../launch/dead-signal/README.md) contains three 1080p screenshots, a 23-second trailer, a cover, upload copy and release notes. The trailer watches a junction come alive, cuts to real recoil combat, shows a red machine rebooting blue, and resolves the original Annex theme on the update title with **Play now**. The boss remains unseen. Stable **3.0.0** includes the restrained “Dead Signal / What’s new” title-screen link; notes open only when requested. The screenshots, trailer link and announcement are public on itch.io.
