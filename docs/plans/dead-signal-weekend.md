...
**Completed validation.** The [balance report](dead-signal-balance.md) replaces the resolved implementation gates: 1,885 passing tests, a production build, 24 paired regional continuations, 50 fresh campaign attempts with 13 full extractions (including two Daily runs), an actual Overtime boarding and first-room clear, 144 stationary probes, 40 build probes, save/Daily compatibility, browser warning inspection, a bounded 2.98.1 performance comparison and repeated-restart caps. Nine pilot navigation timeouts and human/hardware limitations remain explicitly recorded. The candidate ZIP and release notes are prepared locally; stable 2.98.1 is unchanged.

**Remaining release acceptance.** Completed work is removed from this list.

- Review the candidate in ordinary player hands, especially Annex's higher health cost, modest Burst-beam/shell builds and the recorded pilot navigation limits. Automated wins do not establish human difficulty, listening quality or low-end support; retain those limitations if no such testing is available.
- Promote the tested candidate through the existing release procedure, then check About/version, normal startup, reward selection, Report an issue and preserved progress on both public hosts. Verify the actual itch.io iframe version. These public checks cannot be completed against the feature-branch preview.
...
**Launch presentation — staged.** [The launch kit](../launch/dead-signal/README.md) contains three 1080p screenshots, a 23-second teaser, a cover, upload copy and launch-day notes. The teaser watches a junction come alive, cuts to real recoil combat, shows a red machine rebooting blue, and resolves the original Annex theme on the update title. The boss remains unseen. Candidate **3.0.0-rc.2** adds a restrained “Dead Signal / What’s new” title-screen link; its short notes open only when requested. The refreshed 1,885-test suite and production build pass. All presentation work is staged on the feature branch; public promotion remains in the acceptance list above.
