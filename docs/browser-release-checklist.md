...
Completed software fixes, automated lifecycle checks and available-browser measurements are archived in the [performance/stability report](performance-stability-2.96.0.md), including the owner's Edge Quick/Soak runs and Chrome replay check. Use the [repeatable test page and acceptance procedure](performance-testing.md) for the remaining checks. These results do not establish performance on a low-end laptop or full browser/input acceptance.

- [ ] Finish Edge acceptance with a Reduced effects Quick run, gameplay checks and exported-video download/playback. Investigate the remaining isolated frame stalls documented in the [Quick](performance-stability-2.96.0.md#owner-supplied-edge-quick-run) and [Soak](performance-stability-2.96.0.md#owner-supplied-edge-soak-run) results; the longer run did not reproduce the earlier portal/beam stalls or sustained slowdown.
- [ ] Complete production-build stress and gameplay checks in current Chrome and Firefox on Windows, plus Safari if it will be advertised. Include downloading and playing exported replay videos in Firefox. Record actual versions, device specs and results for each browser.
