...
Completed software fixes, automated lifecycle checks and available-browser measurements are archived in the [performance/stability report](performance-stability-2.96.0.md), including the owner's Edge Quick run and Chrome replay check. Use the [repeatable test page and acceptance procedure](performance-testing.md) for the remaining checks. These results do not establish performance on a low-end laptop or full browser/input acceptance.

- [ ] Finish Edge acceptance with a twelve-minute Soak, a Reduced effects Quick run, gameplay checks and exported-video download/playback. Investigate the six frames over 50 ms and ten dropped simulation steps in the [full-effects Quick report](performance-stability-2.96.0.md#owner-supplied-edge-quick-run).
