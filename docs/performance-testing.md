...
From 2.96.1, diagnostic schema 2 also measures audio work, diagnostic housekeeping and the frame callback through room transitions. `frameDiagnostics` records the callback **preceding** an interval over 50 ms. The work done to catch up after a delay is not treated as the cause of that delay. Browser-provided Long Task and Long Animation Frame observations are recorded when available, with overlapping observations attached to the delayed interval. Their presence is correlation, not proof of cause; absent or unsupported observations do not establish that the browser was idle. Rendering, native image work, scheduling and competing applications can fall outside synchronous game timings.

The trace keeps the latest 120 slow intervals and 120 browser entries, reporting discarded counts; each slow interval includes up to eight overlapping entries. Warm-up intervals are retained with `measured: false`. Pause/resume breaks the interval history so hidden time is not labeled a stall. Script URLs, function names and page/container text are not collected. Live output is a compact summary to avoid repeatedly laying out the entire growing trace; pause, stop, completion and the downloaded report expose the full retained detail. API semantics are defined in the [Long Animation Frames draft](https://www.w3.org/TR/long-animation-frames/) and [Long Tasks draft](https://www.w3.org/TR/longtasks-1/).
