...
Selecting a key starts capture. Duplicate bindings leave the old assignment intact and name the conflict. Escape cancels capture, Tab cancels and moves focus, and losing focus cancels it. Escape, Tab, Enter, reward-number keys, modifier shortcuts and browser function keys stay reserved. Restore default keys resets only bindings. Invalid saved mappings fall back to a complete usable map. Controls, contextual hints, warm-up tips, canvas labels and retry tooltips follow saved bindings. Native menu keys retain their normal behavior even if assigned to gameplay actions.
